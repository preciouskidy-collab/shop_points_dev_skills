...
├── skills/                    # 可执行的 Skill（仅 req-to-dev + common）
│   ├── config.js              # 类别定义
│   ├── req-to-dev/            # 主流程：飞书 PRD → 需求 → 编码 → 验证
│   │   ├── SKILL.md
│   │   ├── scripts/           #   run_workflow.py + feishu_fetcher.py
│   │   ├── references/        #   tech-design-template / coding-standards / keboot-project-structure
│   │   └── sub_skills/
│   │       └── feishu-doc-fetcher/
│   └── common/                # 元工具
│       └── skill-creator/
...
req-to-dev 是唯一的主流程入口，一条命令跑通从 PRD 到代码验证的全流程：

```
飞书 PRD → 需求拆解 → 范围评估 → [审批] → 技术方案 → 编码 → 审查 → 测试 → 发布验证
```

各阶段引用 guardrails/knowledge/playbooks 提供规范支撑：

| 阶段 | 动作 | 引用 |
|------|------|------|
| 1. PRD 拉取 | feishu-doc-fetcher | — |
| 2. 需求拆解 | requirement-breakdown | knowledge（系统上下文） |
| 3. 范围评估 | scope-evaluation | knowledge（模块/数据库） |
| 4. 人工审批 | plan-approve | — |
| 5. 技术方案 | 按 PRD + impact 生成 | knowledge |
| 6. 编码 | 按 tech-design 生成代码 | guardrails（分层/编码规范） |
| 7. 审查 | review-checklist | guardrails |
| 8. 测试 | test-authoring | checkpoints |
| 9. 发布验证 | release-validation | — |
