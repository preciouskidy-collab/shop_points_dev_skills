...
`prd_resync.py` 是**侧车脚本**，按 Tier **可回退 `current_stage`**（v0.6.1+ 强制策略）：

| Tier | 回退目标 | 人工审批 |
|------|----------|----------|
| 1 | `plan-approve`（**仅当已过** plan-approve，如 E2E 后） | **轻量重审**：改造点 / UI·E2E，不重审协议 |
| 2 | `plan-approve`（若已过审批点） | **全量重审**：协议 + 详设 |
| 3 | `scope-eval`（若已过范围评估） | 重跑分析链 → plan-approve |

回退后 `plan-approve` 为 blocking；`approve` 成功会清除 `handoff_stale` / `needs_collab_reapprove`。

### 6.2 Tier 分级（LLM 语义分析）

`prd_resync.py` 调用 `lib/prd_tier.py`，由 LLM 结合 **patch plan + PRD diff + 当前 stage + impact 摘录** 判定 Tier（**非关键词规则**）。产物 `collaboration/patch-NNN/tier_analysis.json` 可审计。

```mermaid
flowchart TD
  DIFF[PRD diff + plan.json] --> LLM[LLM 语义分级]
  LLM --> T1[Tier-1 文案/UI]
  LLM --> T2[Tier-2 接口/字段/规则]
  LLM --> T3[Tier-3 范围变更]

  T1 --> U1[spec.md + tasks.md delta]
  T2 --> U2[spec + tasks + handoff_stale=true]
  T3 --> U3[impact.md 附录 + needs_collab_reapprove]
```

| Tier | 典型变更 | 更新范围 | Pipeline stage |
|------|----------|----------|----------------|
| 1 | 文案、空态、圆角 | spec + tasks | **不变**，继续 coding |
| 2 | 字段、错误码、业务规则 | + handoff 过期标记 | **不变**，需补跑契约对齐 |
| 3 | 新页面、新服务 | + impact 附录 | **不变**，建议重新 plan-approve |

**前置条件**：`secrets.local.json` 须配置 `llm.api_key`（与 digest/meeting 共用网关）。

### 6.3 prd_resync 后进入哪个环节

```mermaid
flowchart LR
  RS[prd_resync 完成] --> T{Tier?}
  T -->|1| C[current_stage 不变]
  T -->|2| PA[回退 plan-approve 阻塞]
  T -->|3| SE[回退 scope-eval 重跑链]
  PA --> AP[approve 后继续编码]
  SE --> CH[scope-eval → … → plan-approve]
```

> **注意**：Tier-2/3 回退后须先完成重审（`approve`），不要跳过审批直接 advance。
